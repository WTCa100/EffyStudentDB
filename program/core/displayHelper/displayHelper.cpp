...
namespace Core::Display
{
    ...
    void Helper::displayGrades(const uint16_t& studentId) const
    {
        ...
        for(const auto& grade : grades)
        {
            ...
            {
                const std::shared_ptr<Subject> currentSubject = std::dynamic_pointer_cast<Subject>(dataSubjects->at(targetSubjectId));
                std::cout << currentSubject->name_ << ": " << grade.value_ << "\n";
            }
            else
            {
                LOG((*logger_), "No such subject in session: ", grade.subjectId_, " found in session data. Internal subject name = ", grade.subjectName_);
                std::cout << grade.subjectId_ << " ERROR: No such subject found in Session data! (Internal info only: grade.value_ = " << grade.value_ << " grade.subjectName_ " << grade.subjectName_ << ")";
            }
        }
    }

    Helper::Helper(std::shared_ptr<Logger> logger, std::shared_ptr<SqlAdapter> sqlAdapter, std::shared_ptr<SessionData> sessionData) : logger_(logger), sAdapter_(sqlAdapter), data_(sessionData)
    {
