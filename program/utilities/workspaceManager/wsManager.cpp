...
namespace Utilities
{
    WsManager::WsManager(std::string workingDir):
        workingDir_(std::filesystem::path(workingDir))
    {
        ...
        if (!dManager_->exist(directoryLogs)) { dManager_->createDirectory(directoryLogs); }
    ...
    void WsManager::initializeDatabase()
    {
        ...
        if (createFile(fileBase, fileBaseSubdir))
        {
            ...
            if (!fileExists(fileDatabase, fileDatabaseSubdir))
            {
                ...
                if (!sManager_->moveSchemasToDatabase()) { LOG((*logger_), "Could not add inital schemas into the database!"); }
                else { LOG((*logger_), "All schemas have been moved into the database!"); }
            }
        }
    ...
    {
        return (!dManager_->exist(directoryDatabase) || !dManager_->exist(directorySchemas, directorySchemasSubdir) ||
                !fManager_->exist(fileBase, fileBaseSubdir) || !fManager_->exist(fileDatabase, fileDatabaseSubdir));
    }
    ...
    {
        LOG((*logger_), "Deleting file: fileName=", fileName, " subPath=", (subPath.has_value() ? subPath.value() : "nullopt"),
    ...
    bool WsManager::createDirectory(std::string directoryName, std::optional<std::filesystem::path> subPath)
    {
        ...
        LOG((*logger_), "Creating directory: directoryName=", directoryName,
            " subPath=", (subPath.has_value() ? subPath.value() : "nullopt"));
        // Check for folder
        bool isCreated;
        ...
        if (isCreated) { LOG((*logger_), "Directory created without any issues!"); }
        else { LOG((*logger_), "Could not create a given directory") }
    ...
    {
        LOG((*logger_), "Attempting to delete directory: name=", directoryName,
            "subpath=", subPath.has_value() ? subPath.value().string() : "nullopt");
